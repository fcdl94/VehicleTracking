...
## To build the client
The client is in the client folder. To build it uses the build.xml file. First generate the classes relative to the xsd.
There are two clients in the folder, one simulating the behavior of the driver, one for administrator.
In order to launch them, there is the build.xml ANT file, where you find a task for each one.